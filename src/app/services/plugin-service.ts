...
export class PluginService {
  ...
  checkSIMOrEsimAvailable(): Observable<any> {
    var subject = new Subject<any>();
    if (this.constant.getPlatform().toLowerCase() == this.constant.val_android) {
      cordova.plugins.simdetect.isSimAvailableAndroid(function (success) {
        console.log('Android SimDetect => checkSimStatus success', success);
  ...
  checkForRootedDeviceFromDiagnostic(): Observable<any> {
    const subject = new Subject<any>();

    cordova.plugins.diagnostic.isDeviceRooted(function (status) {
  ...
  checkScreenMirroring() {
    console.log("checkScreenMirroring ===========>")
  ...
  checkIfLocationEnabled() {
    ...
    cordova.plugins.diagnostic.requestLocationAuthorization((status) => {
      this.dataService.checkLocationPermissions = false;
      switch (status) {
        case cordova.plugins.diagnostic.permissionStatus.NOT_REQUESTED:
        ...
        case cordova.plugins.diagnostic.permissionStatus.DENIED_ALWAYS:
          ...
          // this.commonMethods.openPopup('div.popup-bottom.location-permission-denied', true)
          this.dfd.resolve(false);
          break;
        case cordova.plugins.diagnostic.permissionStatus.GRANTED:
          console.log("Permission granted always");
          this.proceedToCheckLocationAccurracy().then((status) => {
            if (status) {
    ...
    cordova.plugins.diagnostic.getLocationAuthorizationStatus((status) => {
      switch (status) {
        ...
        case cordova.plugins.diagnostic.permissionStatus.DENIED_ALWAYS:
          ...
          this.dfd.resolve(false);
          // self.commonMethods.openPopup('div.popup-bottom.location-permission-denied', true)
          break;
        case cordova.plugins.diagnostic.permissionStatus.GRANTED:
          console.log("Permission granted always");
          this.dfd.resolve(true);
  ...
  checkWifiConnection() {
    ...
    if (states[networkState] != 'WiFi') {
      this.dataService.networkSwitchedCount = 0;
    }
    console.log('networkSwitchedCount => ', this.dataService.networkSwitchedCount)
    return states[networkState];
  }
}
