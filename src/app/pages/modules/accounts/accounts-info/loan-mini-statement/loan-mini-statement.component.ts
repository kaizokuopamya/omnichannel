...
import { SHAREDETAILOBJ } from 'src/app/model/common.model';
...
@Component({
  selector: 'app-loan-mini-statement',
  templateUrl: './loan-mini-statement.component.html',
  styleUrls: ['./loan-mini-statement.component.scss']
})
export class LoanMiniStatementComponent implements  OnInit{
  ...
  constructor(
    ...
    private loanMiniStatementService: LoanMiniStatementService,
  ...
  initialization() {
    ...
    let req = this.loanMiniStatementService.getRecommendedOffersReq();
    this.getRecommendedOffers(req);
    let param = this.loanMiniStatementService.getRepaymentStatusParam(this.loanDetails.accountNo);
    this.getRepaymentStatus(param);

    var assessmentParam = this.loanMiniStatementService.getAssessmentYearCall();
  ...
  getLoanEnq(accNo) {
    var param = this.loanMiniStatementService.getMyLoansInquiry(accNo, this.selAccDtl.branchCode);
  ...
  getLoanMiniStatement(accNo) {
    ...
    this.recordNotFound = '';
    var param = this.loanMiniStatementService.getMyLoansMiniStatement(accNo);
  ...
  */
  ...
  public SavePDF(elementRef, fileName): void {
    ...
    (doc as any).fromHTML(content.innerHTML, 20, 20, {
      width: 500,
  ...
  generatePDF(print?: any) {
    ...
    doc.text("Branch Code : " + this.loanUserDtl.branchPinCode, pageWidth - 110, 15, { align: 'left' });
    doc.text("Branch Address : " + this.loanUserDtl.BRANCHADDRESS, pageWidth - 110, 20, { align: 'left' });
    doc.text("Branch Contact : " + this.loanUserDtl.phone_number, pageWidth - 110, 25, { align: 'left' });
    doc.text("IFSC : " + this.loanUserDtl.ifscCode, pageWidth - 110, 30, { align: 'left' });
    ...
    doc.setFontSize(10);
    doc.text("as on date : " + this.datePipe.transform(new Date(), 'dd/MM/yyyy'), 70, 60, { align: 'left' })

    doc.setLineWidth(0.2);
    doc.rect(15, 70, doc.internal.pageSize.width - 30, 50, 'S');

    doc.setFontSize(15);
    // doc.text("Account Name :"+this.loanUserDtl?.customerName, 20, 80, {align :'left'});

    doc.setFontSize(10);
    doc.text("Customer ID : " + this.loanUserDtl?.customerID, 20, 85, { align: 'left' });
    doc.text("Account Number : " + this.selectedAccountNo, 20, 90, { align: 'left' });
    doc.text("Account Holder Name : " + this.loanUserDtl?.accountName, 20, 95, { align: 'left' });
    ...
    // doc.text("Account Open Date : "+ this.Date, 20, 115, {align :'left'});

    // doc.setLineWidth(0.5);
    // doc.line(pageWidth/2, 80, pageWidth/2, 110);

    doc.text("Account Category : " + this.selAccDtl?.accountType + "-" + this.selAccDtl?.schemeDescription, pageWidth - 100, 85, { align: 'left' });
    doc.text("Date : " + this.datePipe.transform(new Date(), 'dd/MM/yyyy'), pageWidth - 100, 90, { align: 'left' });
  ...
  dnldInterestCertificatelatra() {
    ...
    var param = this.loanMiniStatementService.getInterestCertificateParamlatra(this.selAccDtl.accountNo);
  ...
  provisionalCertificate() {
    ...
    var param = this.loanMiniStatementService.getprovisionalCertificate(this.selAccDtl.accountNo, this.customerid);
  ...
  dwnldloanprovisional() {
    ...
    doc.addFont("Sakalbharati.ttf", "Sakalbharati", "normal");
    doc.setFont("Sakalbharati");
    doc.text("Branch Name : " + this.loanUserDtl.branch_name, pageWidth - 115, 20, { align: 'left' });
    var splitTitles = doc.splitTextToSize("Address : " + this.loanUserDtl.BRANCHADDRESS, 50);
    doc.text(splitTitles, pageWidth - 115, 25, { align: 'left' });
    // doc.text("Address : "+this.loanUserDtl.branch_name, pageWidth - 115, 25, {align :'left'});
    doc.setLineWidth(0.2);
    ...
    doc.text("IFSC Code : " + this.loanUserDtl.ifscCode, pageWidth - 55, 25, { align: 'left' });
    doc.text("MICR Code : " + this.loanUserDtl.micrCode, pageWidth - 55, 30, { align: 'left' });
    doc.text("Telephone: " + this.loanUserDtl.phone_number, pageWidth - 55, 35, { align: 'left' });
    ...
    doc.setFont("Sakalbharati");
    doc.text("Provisional Loan Interest Certificate", pageWidth / 2, 60, { align: 'center' });
    ...
    doc.addFont("Sakalbharati.ttf", "Sakalbharati", "normal");
    doc.setFont("Sakalbharati");
    doc.text(this.loanAccIssueDate, 155, 65, { align: 'left' });
    // doc.text(this.interestCertficate[0].custName, 15, 70, {align :'left'});
    doc.text("Cusomer Name: " + this.interestCertficatelatradatea.customerName, 15, 75, { align: 'left' });
    doc.text(this.loanUserDtl?.branch_name, 15, 80, { align: 'left' });
    doc.text("Customer Id: " + this.interestCertficatelatradatea.customerId, 15, 85, { align: 'left' });
    doc.text("PAN: " + this.dataService.profiledateDetails[0].panNumber, 15, 90, { align: 'left' });
    doc.text("Dear Customer,", 15, 100, { align: 'left' });
    // doc.text("Please find below the provisional figure of the repayment done/to be done against Loan Account Number 01231200000123 in the name Borrower Name Co=Borrower name for the period April 01,2021 to March 31,2022,", 15, 105, {align :'left'});
    ...
    doc.setFontSize(10);
    doc.text("The breakup towards the principal and interest is given below: ", 15, 143, { align: 'left' });
  ...
  dnldInterestCertificate() {
    ...
    var param = this.loanMiniStatementService.getInterestCertificateParam(this.interestCertificateForm.value, selAccDtl, this.selType);
  ...
  dnldInterestCertificateSecond() {
    ...
    if (this.interestCertificateForm.valid) {
      ...
      var selAccDtl = this.totalAccountList.filter(item => item.accountNo == this.interestCertificateForm.value.accNo);
      var param = this.loanMiniStatementService.getInterestCertificateParamSecond(this.interestCertificateForm.value, selAccDtl);
      this.http.callBankingAPIService(param, this.storage.getLocalStorage(this.constant.storage_deviceId), this.constant.serviceName_LAIRACCOUNTINQUIRY).subscribe(data => {
        var resp = data.responseParameter;
  ...
  dwnldInterestCertificate() {
    ...
    doc.setFontSize(12);
    doc.setFont("Sakalbharati", "bold");
    doc.text("Loan Interest Certificate", pageWidth / 2, 60, { align: 'center' });

    doc.setFontSize(10);
    doc.setFont("Sakalbharati", "normal");
    // doc.text(this.datePipe.transform(new Date(), 'dd-MMM-yyyy'), 155, 65, {align :'left'});
    doc.text(this.loanAccIssueDate, pageWidth - 60, 65, { align: 'left' });
    // doc.text(this.interestCertficate[0].custName, 15, 70, {align :'left'});
    doc.text(this.intCertificate[0]?.customerName, 15, 75, { align: 'left' });
    doc.text(this.loanUserDtl?.branch_name, 15, 80, { align: 'left' });
    doc.text("Customer Id: " + this.intCertificate[0].customerId, 15, 85, { align: 'left' });
    doc.text("PAN: " + this.dataService.profiledateDetails[0].panNumber, 15, 90, { align: 'left' });
    doc.text("Dear Customer,", 15, 100, { align: 'left' });
    ...
    doc.text(splitTitle, 15, 135, { align: 'left' });
    ...
    doc.setFontSize(10);
    doc.text("The breakup towards the principal and interest is given below: ", 15, 157, { align: 'left' });
  ...
  dwnldBalanceCertificate() {
    ...
    var param = this.loanMiniStatementService.getBalanceCertificateParam(selAccDtl);
  ...
  dwnldBalanceCertificatePdf() {
    ...
    doc.text("Branch Address : " + this.selAccDtl.BRANCHADDRESS, pageWidth - 110, 20, { align: 'left' });
    doc.text("Branch Contact : " + this.selAccDtl.phone_number, pageWidth - 110, 25, { align: 'left' });
    doc.text("IFSC : " + this.selAccDtl.ifscCode, pageWidth - 110, 30, { align: 'left' });
    // doc.text("MICR Code : ", pageWidth - 110, 35, {align :'left'});
    ...
    doc.setFontSize(20);
    doc.text("Balance Certificate (Account wise)", 20, 60, { align: 'left' });
    doc.setFontSize(10);
    doc.text(this.downloadBalCertDateOfIssue, pageWidth - 60, 60, { align: 'left' });
    ...
    doc.text("Dear Sir /Madam", 20, 130, { align: 'left' });
    doc.text("This is to certify that the undernoted balance /s in the account number " + this.balanceCertificateData[0]['0'] + " of the", 20, 140, { align: 'left' });
    doc.text("Above mentioned is available as on date " + this.downloadBalCertDateOfIssue.split(' ')[0] + " at :" + this.downloadBalCertDateOfIssue.split(' ')[1], 20, 145, { align: 'left' });
  ...
  shareAccountDtl() {
    ...
    if (this.constant.getPlatform() != "web") {
      // window.plugins.socialsharing.share(details); //NEED TO DISCUSS WITH VIVEK/JIJO
    }
    else {
