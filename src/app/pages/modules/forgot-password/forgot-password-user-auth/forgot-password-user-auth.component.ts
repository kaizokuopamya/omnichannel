...
export class ForgotPasswordUserAuthComponent implements OnInit {
  ...
  maskedEmail: any = '';
  maskedMobile: any = '';
  maskMobileError: boolean = false;
  maskEmailError: boolean = false;
  ...
  submitForm() {
    ...
    this.maskMobileError = false;
    this.maskEmailError = false;
    ...
    if (this.forgotpasswordauthForm.valid) {
      ...
      if (this.selectedtab == 'card') {
        ...
        if(this.userDetail.mobileNumber == this.forgotpasswordauthForm.get('mobile').value && this.userDetail.emailId.toLowerCase() == this.forgotpasswordauthForm.get('email').value.toLowerCase()){
          this.resendLeadOTP(this.forgotpasswordauthForm.get('mobile').value, this.forgotpasswordauthForm.get('email').value.toLowerCase());
        }
        else {
          if(this.userDetail.mobileNumber != this.forgotpasswordauthForm.get('mobile').value){
            this.maskMobileError = true;
          }
          if(this.userDetail.emailId.toLowerCase() != this.forgotpasswordauthForm.get('email').value.toLowerCase()){
            this.maskEmailError = true;
          }
        }
      }
    }
  ...
  validateOtp() {
    if (this.otpForm.valid) {
      ...
      let deviceID = this.constant.deviceID;
      var rrnNo = this.commonMethods.genRandomDigit(9); 
      var param = this.forgotPasswordUserAuthService.getChannelLeadOtpParam(mobileOtp, emailOtp, this.resendOtpLinkMobNumber, this.resendOtpLinkEmail , rrnNo);
  ...
  resendLeadOTP(mobileNo, emailId) {
    var resendOTPReq = this.forgotPasswordUserAuthService.getResendLeadOtpParam(mobileNo, emailId,this.constant.val_FORGOTPASSWORD);
    this.http
      .callBankingAPIService(
        resendOTPReq,
  ...
  resendOTPLink(type) {
    ...
    var resendOTPReq = this.forgotPasswordUserAuthService.getResendLeadOtpParam(this.resendOtpLinkMobNumber, this.resendOtpLinkEmail, this.constant.val_FORGOTPASSWORD,type.toLowerCase());
    this.http
      .callBankingAPIService(
        resendOTPReq,
        this.constant.deviceID,
