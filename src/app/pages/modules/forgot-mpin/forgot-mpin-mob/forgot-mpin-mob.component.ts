...
export class ForgotMpinMobComponent implements OnInit {
    ...
    if (this.activeTab == 'step1') {
      ...
      if (this.mpinForm.valid) {
        ...
        if (!this.confMpinError && this.mpinMatch && !this.validConfirmMpin && !this.mpinError && !this.validMpin) {
         ...
         var param1 = this.regService.getForgotMpinParam(mpin);
         this.setMpin(param1);
        }
        // else {
  ...
  validateUserDetails(param, deviceId) {
    ...
    this.http.callBankingAPIService(param, deviceId, this.constant.serviceName_VERFYCREDNTIALS,extraParams).subscribe(data => {
      console.log(data);
      var resp = data.responseParameter
      if (resp.opstatus == "00") {
  ...
  resendLeadOTP(mobileNo, emailId) {
    var resendOTPReq = this.forgotPasswordUserAuthService.getResendLeadOtpParam(mobileNo, emailId,this.constant.val_FORGOTMPIN);
    this.http
      .callBankingAPIService(
        resendOTPReq,
  ...
  setMpin(param) {
    this.http.callBankingAPIService(param, this.storage.getLocalStorage(this.constant.storage_deviceId), this.constant.serviceName_SETUPDATEPIN).subscribe(data => {
      ...
      if (resp.opstatus == "00") {
        console.log(data.responseParameter);
        this.dataService.rrnNo = data.RRN
  ...
  resendOTPLink(type) {
    ...
    var resendOTPReq = this.forgotPasswordUserAuthService.getResendLeadOtpParam(this.resendOtpLinkMobNumber, this.resendOtpLinkEmail,this.constant.val_FORGOTMPIN,type);
    this.http
      .callBankingAPIService(
        resendOTPReq,
        this.constant.deviceID,
